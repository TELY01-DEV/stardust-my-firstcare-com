    ...
    def process_message(self, topic: str, payload: str):
        ...
        try:
            ...
            if topic == "ESP32_BLE_GW_TX":
                ...
                # Log failed transaction for unknown topic
                self.transaction_logger.log_transaction(
                    operation="ava4_message_processing",
                    data_type="vital_signs",
                    collection="patients",
                    status="failed",
                    details=f"Unknown topic: {topic}",
                    device_id="unknown"
                )
    ...
    def process_ava4_status(self, data: Dict[str, Any]):
        ...
        try:
            ...
            if not mac_address:
                ...
                # Log failed transaction
                self.transaction_logger.log_transaction(
                    operation="ava4_status_processing",
                    data_type="device_status",
                    collection="patients",
                    status="failed",
                    details="No MAC address in AVA4 status message",
                    device_id="unknown"
                )
                return
            ...
            if not patient:
                ...
                self.transaction_logger.log_transaction(
                    ...
                    collection="patients",
                    status="failed",
                    details=f"No patient found for AVA4 MAC: {mac_address}",
            ...
            self.transaction_logger.log_transaction(
                ...
                status="success",
                details=f"AVA4 status: {msg_type}",
                device_id=mac_address
            )
            
            # Store device status (optional - for monitoring)
            logger.info(f"DEBUG: About to create status_data for AVA4 {mac_address}")
            status_data = {
                "device_id": mac_address,  # Use MAC address as device_id for AVA4
                "device_type": "ava4",
                "imei": imei,
                "mac_address": mac_address,  # Include MAC address field
                "online_status": "online" if msg_type == "HB_Msg" else "offline",
                "last_updated": datetime.utcnow(),
                "patient_id": patient['_id'],
                "message_type": msg_type,
                "data": data.get('data', {}),
                "battery_level": None,  # AVA4 doesn't provide battery level
                "signal_strength": None,  # AVA4 doesn't provide signal strength
                "alerts": []
            }
            
            logger.info(f"DEBUG: Status data created for AVA4 {mac_address}, about to store in latest_devices_status")
            
            # Store this in latest_devices_status collection
            try:
                logger.info(f"Attempting to store device status for AVA4 {mac_address} in latest_devices_status collection")
                ...
                if result.upserted_id:
                    logger.info(f"✅ Created new device status record for AVA4 {mac_address} in latest_devices_status collection")
                elif result.modified_count > 0:
                    logger.info(f"✅ Updated existing device status record for AVA4 {mac_address} in latest_devices_status collection")
                else:
                    logger.info(f"✅ Device status record for AVA4 {mac_address} already up-to-date in latest_devices_status collection")
                    
            except Exception as e:
                logger.error(f"❌ Failed to store device status for AVA4 {mac_address} in latest_devices_status collection: {e}")
                # Log failed transaction for device status storage
                self.transaction_logger.log_transaction(
                    operation="ava4_device_status_storage",
                    data_type="device_status",
            ...
            self.transaction_logger.log_transaction(
                ...
                collection="patients",
                status="failed",
                details=f"Error processing AVA4 status: {e}",
    ...
    def process_ava4_medical_data(self, data: Dict[str, Any]):
        ...
        try:
            ...
            if success:
                ...
                # Log successful transaction
                self.transaction_logger.log_transaction(
                    operation="ava4_medical_data_processing",
                    data_type="vital_signs",
                    collection="patients",
                    patient_id=str(patient['_id']),
                    status="success",
                    details=f"Successfully processed {attribute} data",
                ...
                self.transaction_logger.log_transaction(
                    ...
                    patient_id=str(patient['_id']),
                    status="failed",
                    details=f"Failed to process {attribute} data",
                    device_id=device_mac
                )

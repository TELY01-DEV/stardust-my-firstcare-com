    ...
    def find_patient_by_kati_imei(self, imei: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if watch and watch.get("patient_id"):
                # Convert patient_id to ObjectId properly handling different formats
                patient_id = watch["patient_id"]
                
                # Handle different patient_id formats
                if isinstance(patient_id, str):
                    try:
                        # Check if it's MongoDB extended JSON format
                    ...
                    try:
                        ...
                        logger.warning(f"Invalid ObjectId format in dict: {patient_id}")
                        return None
                elif isinstance(patient_id, ObjectId):
                    # Already an ObjectId
                    pass
                else:
                    logger.warning(f"Unexpected patient_id format: {type(patient_id)} - {patient_id}")
                    return None

...
def analyze_ava4_event_logs(client):
    """Analyze AVA4 event logs to extract sub-device MAC addresses"""
    try:
        db = client[MONGODB_DATABASE]
        event_logs = db.event_logs
        
        # Get AVA4 events with payload details
        ava4_events = list(event_logs.find({
            'source': 'AVA4',
            'details.payload': {'$exists': True}
        }).sort('timestamp', -1).limit(50))
        
        print(f"📊 Found {len(ava4_events)} AVA4 events with payload")
        
        sub_device_mappings = {}
        
        for event in ava4_events:
            try:
                payload = event.get('details', {}).get('payload', {})
                
                # Check if this is a sub-device data payload
                if 'data' in payload and 'value' in payload.get('data', {}):
                    data = payload['data']
                    value = data.get('value', {})
                    
                    # Extract sub-device information
                    if 'device_list' in value:
                        device_list = value['device_list']
                        
                        for sub_device in device_list:
                            ble_addr = sub_device.get('ble_addr', '').lower()
                            if ble_addr:
        ...
        print(f"❌ Error analyzing AVA4 event logs: {e}")
        return {}
...
def main():
    ...
    print("🔧 Sub-Device Mapping Database Fix")
    print("=" * 50)
    ...
    # Step 3: Analyze AVA4 event logs
    print("\n3. Analyzing AVA4 event logs for sub-devices...")
    sub_device_mappings = analyze_ava4_event_logs(client)
    ...
    if successful_mappings > 0:
        ...
        print("\n💡 Expected Results:")
        print("- AVA4 sub-devices (blood pressure, etc.) will show patient names")
        ...
        print("2. Verify AVA4 event logs contain proper payload structure")

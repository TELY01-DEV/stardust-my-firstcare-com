#!/usr/bin/env python3
"""
LATEST AMY DATABASE IMPORT UTILITY (October 25, 2024 Data)
==========================================================
Import the latest AMY_25_10_2024 JSON files to update the production database
"""
...
def find_amy_json_files():
    """Find all AMY_25_10_2024 JSON files"""
    downloads_path = "/Users/kitkamonm/Downloads"
    pattern = os.path.join(downloads_path, "AMY_25_10_2024.*.json")
    
    json_files = glob.glob(pattern)
    print(f"🔍 Found {len(json_files)} AMY_25_10_2024 JSON files:")
...
def main():
    """Main import function for latest AMY data"""
    print("🚀 LATEST AMY DATABASE IMPORT UTILITY")
    print("=" * 80)
    print(f"🕒 Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"📅 Importing data from: October 25, 2024")
    print()
    
    # Find all AMY_25_10_2024 JSON files
    json_files = find_amy_json_files()
    
    if not json_files:
        print("❌ No AMY_25_10_2024 JSON files found in Downloads folder")
    ...
    proceed = input(f"\n🎯 Proceed with importing {len(json_files)} collections? (type 'YES' to confirm): ")
    if proceed != 'YES':

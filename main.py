...
# Clear startup log to confirm code execution
print("🚀 MAIN.PY STARTUP: Code execution confirmed at startup")
print("🚀 MAIN.PY STARTUP: This confirms the latest main.py is being executed")
...
from app.routes.transaction_logging import router as transaction_logging_router
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Create TTL indexes for audit logs (optional - only if FHIR DB is available)
        try:
            ...
            logger.warning(f"⚠️ Could not create FHIR audit TTL index: {e}")
            logger.info("ℹ️ Continuing without FHIR audit TTL index")
...
app.include_router(device_status_router, tags=["device-status"]) # has prefix /api/devices/status
...
app.include_router(transaction_logging_router, tags=["transaction-logging"]) # has prefix /admin/transaction-logs

# Diagnostic: Log all registered routes
for route in app.routes:
    try:
        logger.info(f"Registered route: {getattr(route, 'path', str(route))}")
    except Exception as e:
...
# Device Status Dashboard route
@app.get("/device-status-dashboard", include_in_schema=False)
async def device_status_dashboard():
    """Serve the device status dashboard HTML page"""
    try:
        dashboard_path = "deployment/opera-godeye-panel/device-status-dashboard.html"
